...
gen_geovision: proto_gen
	@echo "--- Injecting Custom Tags (protoc-go-inject-tag) ---"
	@# Find all .pb.go files in the destination and inject tags
	find $(DESTDIR) -name "*.pb.go" -exec protoc-go-inject-tag -input={} \;
